...
export const MarkdownMessage = ({ content }) => {
	// Flatten children recursively to get full code content
	const flattenCode = children => {
		if (typeof children === 'string') return children;
		if (Array.isArray(children)) return children.map(flattenCode).join('');
		if (children.props && children.props.children) return flattenCode(children.props.children);
		return '';
	};
	...
	return (
        ...
        prose-code:text-sm prose-code:rounded prose-code:bg-gray-100 dark:prose-code:bg-[#424242] 
        prose-pre:bg-[#272822] prose-pre:text-gray-100 prose-pre:rounded-lg 
			...
			<ReactMarkdown
				...
				components={{
					code({ inline, className, children, ...props }) {
						...
						const codeText = flattenCode(children);
						...
						if (inline || !match) {
							return (
								<code
									className="bg-gray-100 dark:bg-[#424242] text-[90%] font-mono rounded px-1"
									{...props}
								>
									{children}
								</code>
						...
						return (
							<div className="relative not-prose my-1">
								{/* Top bar */}
								<div className="absolute top-0 right-0 left-0 flex items-center justify-between text-xs text-gray-400 dark:text-gray-300 px-2 py-1">
									<span className="font-mono mr-2">{language || 'code'}</span>
									<div class="bg-token-bg-elevated-secondary text-token-text-secondary flex items-center gap-4 rounded-sm px-2 font-sans text-xs">
										{copied ? (
											<button class="flex gap-1 items-center select-none py-1" aria-label="Copied">
												<svg
													width="20"
													height="20"
													viewBox="0 0 20 20"
								...
								{/* Code block */}
								<pre className="bg-[#272822] text-gray-100 rounded-md pt-10 px-3 overflow-x-auto">
